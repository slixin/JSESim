...
function JseDerivRuler(market, log) {
    var self = this;
    ...
    var monitor_stop_orders = function() {
        var stop_orders = self.orders.filter(function(o) { return o.status != "CLOSED" && parseInt(o.data.Container) == 6 && o.data.OrderStatus == 0 && parseFloat(o.data.TrailingOffset) == 0 });
    ...
    var monitor_trailing_orders = function() {
        var trailing_orders = self.orders.filter(function(o) { return o.status != "CLOSED" && parseInt(o.data.Container) == 6 && o.data.OrderStatus == 0 && parseFloat(o.data.TrailingOffset) > 0 });
    ...
    var get_match_orders_on_leg = function(leg1_match_orders, leg2_match_orders, diff_price) {
        var pairorders = [];

        leg1_match_orders.forEach(function(l1Order) {
            var leg2_price = parseFloat(l1Order.data.LimitPrice + diff_price);
            var match_orders_on_leg2 = leg2_match_orders.filter(function(o) { return o.data.Side == 1 ? parseFloat(o.data.LimitPrice) >= parseFloat(leg2_price) : parseFloat(o.data.LimitPrice) <= parseFloat(leg2_price) });
            if (match_orders_on_leg2.length > 0) {
                pairorders.match_leg1 = l1Order
                pairorders.match_leg2 = match_orders_on_leg2[0];
                return;
            }
        })
    ...
    // Monitor Multi-Leg orders
    var monitor_multileg_orders = function() {
        var multileg_orders = self.orders.filter(function(o) { return o.status != "CLOSED" && parseInt(o.data.MultiLegReportingType) == 3 });

        if (multileg_orders.length > 0) {
            var multileg_order = multileg_orders[0];
            var match_multileg_orders = self.orders.filter(function(o) {
                return o.status != "CLOSED" &&
                parseInt(o.data.MultiLegReportingType) == 3 &&
                o.data.SecurityID == multileg_order.data.SecurityID &&
                o.data.Side == (multileg_order.data.Side == 1 ? 2 : 1)
            });

            var leg1_match_orders = null;
            var leg2_match_orders = null;
            var leg1_order = multileg_order.children[0];
            var leg2_order = multileg_order.children[1];

            leg1_match_orders = self.orders.filter(function(o) {
                return o.status != "CLOSED" &&
                o.data.OrderType == 2 &&
                o.data.SecurityID == leg1_order.data.SecurityID &&
                o.data.Side == (leg1_order.data.Side == 1 ? 2 : 1)
            });

            if (leg1_match_orders.length > 0) { // Leg 1 with matching orders
                leg2_match_orders = self.orders.filter(function(o) {
                    return o.status != "CLOSED" &&
                    o.data.OrderType == 2 &&
                    o.data.SecurityID == leg2_order.data.SecurityID &&
                    o.data.Side == (leg2_order.data.Side == 1 ? 2 : 1) &&
                    o.data.Side == 1 ? parseFloat(o.data.LimitPrice) >= parseFloat(leg2_order.data.LimitPrice) : parseFloat(o.data.LimitPrice) <= parseFloat(leg2_order.data.LimitPrice)
                });

                if (leg2_match_orders.length > 0) { // Leg 2 with matching orders
                    // trade with regular orders
                    var leg1_match_order = leg1_match_orders[0];
                    var leg2_match_order = leg2_match_orders[0];

                    var execQty = leg1_match_order.data.LeavesQuantity > leg2_match_order.data.LeavesQuantity ? leg2_match_order.data.LeavesQuantity : leg1_match_order.data.LeavesQuantity;

                    matchOrder(leg1_order, leg1_match_order, execQty);
                    matchOrder(leg2_order, leg2_match_order, execQty);

                    matchOrder(multileg_order, null, execQty);
                } else {
                    // implied leg 2
                }
            } else {
                ...
                if (leg2_match_orders.length > 0) {
                    // implied leg 1
                } else {
                    if (match_multileg_orders.length > 0) {
                        var match_multileg_order = match_multileg_orders[0];
                        // trading with multi-leg order
                        var leg1_match_multileg_order = match_multileg_order.children[0];
                        var leg2_match_multileg_order = match_multileg_order.children[1];

                        leg1_order.data.MultiLegReportingType = 2;
                        leg2_order.data.MultiLegReportingType = 2;

                        leg1_match_multileg_order.data.MultiLegReportingType = 2;
                        leg2_match_multileg_order.data.MultiLegReportingType = 2;

                        leg1_match_multileg_order.data.LimitPrice = getMarketPrice(leg1_match_multileg_order.data.SecurityID);
                        leg2_match_multileg_order.data.LimitPrice = parseFloat(leg1_match_multileg_order.data.LimitPrice) + parseFloat(multileg_order.data.LimitPrice);

                        leg1_order.data.SecondaryOrderID = multileg_order.data.OrderID;
                        leg2_order.data.SecondaryOrderID = multileg_order.data.OrderID;

                        leg1_match_multileg_order.data.SecondaryOrderID = match_multileg_order.data.OrderID;
                        leg2_match_multileg_order.data.SecondaryOrderID = match_multileg_order.data.OrderID;

                        matchOrder(leg1_order, leg1_match_multileg_order);
                        matchOrder(leg2_order, leg2_match_multileg_order);
                        matchOrder(multileg_order, match_multileg_order);

                        self.orders.push(leg1_order);
                        self.orders.push(leg2_order);
                        self.orders.push(leg1_match_multileg_order);
                        self.orders.push(leg2_match_multileg_order);
                    }
                }
            }
        }
    ...
    var timer_trailing_orders = setInterval(monitor_trailing_orders, 500);
    var timer_multileg_orders = setInterval(monitor_multileg_orders, 500);
    // *****************************************************************
    ...
    var get_multileg_secs = function(code) {
        var securities = [];
        var secs = self.instruments.filter(function(o) { return o.exchangecode == code });
        if (secs.length > 0) {
            var sec = secs[0];
            var symbol = sec.symbol;
            var symbolarray = symbol.split('-').filter(function(el) {return el.length != 0});
            if (symbolarray.length == 3) { // Maybe it is a multi-leg code, for example: ALSI-DEC17-DEC18
                var symbol_one = symbolarray[0] + '-' + symbolarray[1];
                var symbol_two = symbolarray[0] + '-' + symbolarray[2];

                var symbol_one_secs = self.instruments.filter(function(o) { return o.symbol == symbol_one });
                var symbol_two_secs = self.instruments.filter(function(o) { return o.symbol == symbol_two });
                if (symbol_one_secs.length > 0 && symbol_two_secs.length > 0) {
                    securities.push(symbol_one_secs[0]);
                    securities.push(symbol_two_secs[0]);
                }
            }
        }
    ...
    var processNewOrder = function(session, message) {
        ...
        order.data.LastOptPx = 0;
        order.data.Volatility = 0;

        self.orders.push(order);

        var mleg_securities = get_multileg_secs(order.data.SecurityID);
        if (mleg_securities.length > 0) {
            order.data.MultiLegReportingType = 3;
            var leg1_security = mleg_securities[0];
            var leg2_security = mleg_securities[1];

            var multileg_orderid= utils.randomString(null, 9);
            var leg1_orderid = "O"+multileg_orderid+'L1';
            var leg2_orderid = "O"+multileg_orderid+'L2';
            var leg1_order = {
                session: session,
                account: account,
                status: status,
                ...
                data: {
                    ...
                    Side: order.data.Side == 1 ? 2 : 1,
                    SecurityID: leg1_security.exchangecode,
                    TraderMnemonic: order.data.TraderMnemonic,
                    Account: order.data.Account,
                    ExecutionID: "E"+utils.randomString(null, 11),
                    OrderID: leg1_orderid,
                    OrderQuantity: order.data.OrderQuantity,
                    LeavesQuantity: order.data.OrderQuantity,
                    DisplayQuantity: order.data.OrderQuantity,
                    OrderType: 2,
                    TimeInForce: 0,
                    OrderStatus: 0,
                    Container: getContainer(order.data),
                    ExecutionType: "0",
                    OrderBook: 1,
                    IsMarketOpsRequest: 0,
            ...
            order.children.push(leg1_order);
            var leg2_order = {
                session: session,
                account: account,
                status: status,
                ...
                data: {
                    ...
                    SecurityID: leg2_security.exchangecode,
                    TraderMnemonic: order.data.TraderMnemonic,
                    Account: order.data.Account,
                    ExecutionID: "E"+utils.randomString(null, 11),
                    OrderID: leg2_orderid,
                    OrderQuantity: order.data.OrderQuantity,
                    LeavesQuantity: order.data.OrderQuantity,
                    DisplayQuantity: order.data.OrderQuantity,
                    OrderType: 2,
        ...
        switch(order.data.OrderType) {
            ...
            default:
                ...
                _send_dropcopy_message(template.fix.dc_execution_report, order);
                if (order.data.OrderType == 2 && order.data.MultiLegReportingType != 3) { // Limit order
        ...
        if (marketPrice != undefined) {
            ...
            if (order.data.Side == 1) { // Buy order
                if (order.data.LastMarketPrice == undefined) {
                    order.data.StopPrice = marketPrice + parseFloat(trailingOffset);
                } else {
    ...
    var get_child_orders = function(order_id) {
        return self.orders.filter(function(o) { return o.status != 'CLOSED' && o.parent != undefined && o.parent.data.OrderID == order_id });
    }
    ...
    var matchOrder = function(order, match_order, minQty) {
        ...
        var execprice = match_order == undefined ? order.data.LimitPrice : match_order.data.LimitPrice;
    ...
    self.clearIntervals = function(cb) {
        ...
        clearInterval(timer_multileg_orders);
        cb();
    }
}
